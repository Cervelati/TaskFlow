...
function buildCard(ws) {
    ...
    card.className = 'ws-card'; card.dataset.id = ws.id;
    ...
    card.innerHTML = `
        ...
        <div class="ws-card-body">
            <div class="ws-stat"><span class="ws-stat-icon">👥</span><span>${ws.memberCount} ${ws.memberCount === 1 ? 'membro' : 'membros'}</span></div>
            <div class="ws-stat"><span class="ws-stat-icon">📅</span><span>${formatDate(ws.createdAt)}</span></div>
        </div>
        <div class="ws-card-footer">
            <button class="ws-btn ws-btn-primary" onclick="location.href='workspaceBoard.html?id=${ws.id}'">📋 Abrir board</button>
            <button class="ws-btn ws-btn-secondary" onclick="openMembersModal(${ws.id})">👥 Membros</button>
...
async function saveWorkspace() {
    ...
    finally { resetLoading('btn-save-ws', 'save-ws-label', 'save-ws-spinner', state.editingId ? 'Salvar alterações' : 'Criar workspace'); }
}
...
async function confirmDelete() {
    ...
    try {
        ...
        if (!res) return;
        if (res.ok) { closeModal('modal-delete'); toast('Workspace deletado.', 'success'); await loadWorkspaces(); }
        else toast('Erro ao deletar workspace.', 'error');
    } catch { toast('Erro de conexão.', 'error'); }
    finally { resetLoading('btn-confirm-delete', 'delete-label', 'delete-spinner', 'Deletar'); state.deletingId = null; }
}
...
function renderMembers(members) {
    ...
    list.innerHTML = members.map(m => {
        ...
        return `<div class="member-row"><div class="member-avatar">${initials}</div><div class="member-info"><div class="member-name">${escHtml(m.userName)}</div><div class="member-email">${escHtml(m.email)}</div></div><span class="member-role-badge ${roleClass}">${m.role}</span></div>`;
    }).join('');
}
...
async function sendInvite() {
    ...
    try {
        const res = await apiFetch(`/workspaces/${state.invitingWorkspaceId}/invites`, { method: 'POST', body: JSON.stringify({ email }) });
        if (!res) return;
        if (res.ok) { closeModal('modal-invite'); toast('Convite enviado com sucesso!', 'success'); }
...
document.addEventListener('keydown', e => {
    if (e.key === 'Escape') ['modal-workspace','modal-invite','modal-members','modal-delete','logout-overlay'].forEach(id => closeModal(id));
});

function show(id) { document.getElementById(id)?.classList.remove('hidden'); }
function hide(id) { document.getElementById(id)?.classList.add('hidden'); }
function setLoading(btnId, labelId, spinnerId, text) { const btn = document.getElementById(btnId); if (btn) btn.disabled = true; const label = document.getElementById(labelId); if (label) label.textContent = text; document.getElementById(spinnerId)?.classList.remove('hidden'); }
function resetLoading(btnId, labelId, spinnerId, text) { const btn = document.getElementById(btnId); if (btn) btn.disabled = false; const label = document.getElementById(labelId); if (label) label.textContent = text; document.getElementById(spinnerId)?.classList.add('hidden'); }
